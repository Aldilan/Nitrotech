...
$jdph = 8;
$jd = count(query("SELECT * FROM tb_pesananuser WHERE status = 'sudah dibayar'"));
$jh = ceil($jd / $jdph);
$ha = (isset($_GET["halaman"]) ) ? $_GET["halaman"] : 1;
$ad = ($jdph * $ha) - $jdph;

$barang = query("SELECT * FROM tb_pesananuser WHERE status = 'sudah dibayar' LIMIT $ad, $jdph");
...
<div class="container">
			...
			<center>
				<div class="headkat">
					<table>
						...
						<tr>
							<td colspan="2">Pembeli : <span class="stokbarang"><?php echo $row["username"]; ?></span></td>
							<td></td>
							<td class="dlt"><a href="lihatN.php?id=<?php echo $row['id_barang'];?>">Lihat selengkapnya</a></td>
						</tr>
					</table>
				</div>
			</center>
	...
	<div class="sbl">
		<a href="lihatP.php">Lihat jumlah penghasilan</a>
